...
>
> Open-weight models appear only as *scientific controls*, to show that the mechanism is not a
...
5. **A decomposition of what differs between the mechanisms' outputs**, separating loss of the
   feature-to-target association from over-coupling of features, which distinguishes two failure modes
...
buys and where the ceiling is: the release's own distance from the truth.
...
artifacts DP exists to make safe to publish. By post-processing immunity, whoever computes `G` gains
...
What the prior did supply on Adult is visible in the controls' fidelity, not in CoRTeC's margin:
...
mechanism.** The arm above was generated before the rate-mechanism correction of §4.3, step (iv),
...
Both marginal methods lead on 1-way TV: MST at 0.024 against CoRTeC's 0.052 (adjusted p = 0.0024,
...
At adequate `n`, AIM leads its own workloads. Adult, n = 1,950 per method:
...
The auto-configured release leads the hand-tuned one on all three students (+0.031, +0.014, +0.028)
...
## Appendix B — Artifact and code map
...
| pMSE | [`pmse.py`](https://github.com/Calyie/cortec/blob/main/pmse.py) | pMSE ratio with a permutation null |
...
| Model grid | [`run_model_grid.py`](https://github.com/Calyie/cortec/blob/main/run_model_grid.py) | the capability and family sweep of §H.15.3 |
...
| Rejected remediation | [`calibrate_generation.py`](https://github.com/Calyie/cortec/blob/main/calibrate_generation.py) | kept as the record of §H.14's negative result |
...
all land between 0.605 and 0.669 F1**, above every DP baseline, including Opus 5, which reaches that
...
## Appendix E — Ablation: what each design decision bought

![Figure 12](figures/fig11_ablation.png)

**Figure 12.** What each design decision bought: the before → after of the table below, each on the
...
improvement in the mechanism's most load-bearing property cost no privacy budget at all.
...
> 0.613, model trained on all the real data 0.677. Predicting "no diabetes" for every respondent beats
...
If the binding constraint is what the release *contains*, then drawing **more records** from one release
...
First, **the privacy mechanism is not the limiting factor.** The released histograms sit at 0.017,
...
| 19 | The auto-configurator's DP column selection ran with a **hardcoded seed** (`AUTO_SEED = 0`) | It charges `ε_selection` out of the release budget for a query over the private data, and a seeded mechanism is deterministic — its output distribution is a point mass, so the guarantee is vacuous and the budget bought nothing. Every `*_auto` configuration in this paper reproduced exactly, which looked like a feature. The noise is now unseedable, and because a derivation therefore cannot be repeated, the configuration is **persisted as an artifact** and loaded — the same discipline the release follows. The configurations behind this paper's results are frozen on disk with their provenance recorded |
...
([`src/dp_cohorts.py`](https://github.com/Calyie/cortec/blob/main/src/dp_cohorts.py)): its release file carried the **exact size of every joint conditional cell**
...
The allocation channel is measurable from artifacts already on disk. At ε = 2.0 the
