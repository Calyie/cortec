...
**What we find.** We test at ε = 2, where ε is the privacy budget and smaller is stronger, and at n = 300, on three datasets: census, finance, and a national health examination survey.

On fidelity, CoRTeC's 1-way marginal error, the error in each single column's distribution, is within 0.005 of MST's on Adult and finance, and below it on NHANES. It is below the error of a real sample of the same size, and below AIM's, on every dataset. Its 2-way error, over pairs of columns, is below both marginal methods' on every dataset.

On utility, tree models trained on CoRTeC's output match the real-sample floor on every dataset. The floor is the accuracy of a model trained on a real sample of the same size. The linear model matches it on Adult and is within 0.02 of it on finance and NHANES. Models trained on AIM's and MST's output score 0.05 to 0.18 lower in AUC, the area under the ROC curve, which is the utility measure used throughout. Of 35 comparisons against AIM and MST on Adult and finance, corrected for multiple testing with Holm's procedure, 28 favour CoRTeC, one favours a baseline, MST's 0.005 advantage on 1-way error on Adult, and six do not separate. AIM keeps the lowest error on its own 3-way workloads, sets of three-column marginals, at adequate sample size, which its design predicts.
