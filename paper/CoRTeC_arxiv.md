...
The generative models trained under differential privacy [24, 38, 49] are evaluated on downstream utility, the accuracy of a model trained on the synthetic data and tested on real data. On Adult at ε = 2 they do not preserve the target's base rate.
...
On utility, tree models trained on CoRTeC's output do not separate from the real-sample floor on any of the three datasets, at three draws each; the powered six-draw test is Adult's. The floor is the accuracy of a model trained on a real sample of the same size. The linear model matches it on Adult and is within 0.021 of it on finance and NHANES. Models trained on AIM's and MST's output score 0.05 to 0.18 lower in AUC, the area under the ROC curve, which is the utility measure used throughout. Of 35 comparisons against AIM and MST on Adult and finance, corrected for multiple testing with Holm's procedure, 28 favour CoRTeC, one favours a baseline, MST's 0.005 advantage on 1-way error on Adult, and six do not separate. AIM keeps the lowest error on its own 3-way workloads, sets of three-column marginals, at adequate sample size, which its design predicts.

Two results corrected our own framing. A private relationship forced into the data survives MST essentially perfectly (slope 0.998 on Diabetes 130, 0.946 on NHANES), so the marginal methods' utility gap is not an inability to carry conditional structure: on NHANES, the one dataset with both measurements, MST carries the forced rate and still trains the random-forest student 0.086 AUC lower. On Adult their loss is measurable instead as a 2.2 to 2.8× inflation of pairwise feature dependence relative to real data of the same size; CoRTeC inflates it by 1.24×.
...
- A privacy-budget allocation that makes conditional structure cheap. Public stratification costs nothing; histograms replace moment queries; one histogram block per outcome class costs what one pooled block does; and parallel composition, under which statistics on non-overlapping groups each spend the full budget rather than sharing it, applies across disjoint cohorts and cells. The result is 6.7× more budget per released statistic than a natural implementation, at unchanged ε (Section 4).
...
A histogram over any number of bins has $Δ = 1$, because one record moves exactly one count by one. A counting query has $Δ = 1$ for the same reason. The mechanism is built on this fact: an entire distributional shape costs the same budget as a single count. The total error of a histogram still grows with its bin count, because every bin carries its own noise; what the sensitivity bound fixes is the budget, not the error.
...
An unconditioned "header-only" prompt, the same frozen model given nothing but the column names, appears as a control and not as a competitor. It carries no privacy guarantee. Its role is to establish that CoRTeC's output depends on the released statistics rather than on the model's prior. Two forms appear. The header-only control gives the model only the column names; it is the control behind the aggregate-utility numbers of Section 6.3. The matched header-only control is CoRTeC's own prompt with the released blocks removed and nothing else changed (Appendix A); it is the control behind the inversion test and the registry diagnostic of Section 6.3.
...
Every metric is computed by one implementation that bins numerical columns with the public bin edges every method is fed. One convention is declared. The evaluator's intervals are right-closed, while the release, the selection step and the baselines' discretisation are left-closed, so an integer that lies exactly on an edge counts in different bins under the two. The effect is small and falls on every method. On Adult, 1-way error under the release's convention is 0.020 for CoRTeC, 0.015 for MST, 0.030 for AIM and 0.043 for a real sample, against 0.029, 0.024, 0.037 and 0.043 under the evaluator's. Every synthetic-output number in this paper is the evaluator's; a release's own distance from the training data (Section 6.1, Table 9) is stated under the release's convention. The ordering of methods is the same under either.
...
The release's 1-way error against the training data is 0.009, and the selected draws' 1-way error against the release is 0.002 (both under the release's convention). The marginal result therefore carries over to a release nobody tuned: 1-way error 0.015 against the sample's 0.034, conditional error over seen groups 0.006 against 0.015, held-out 0.036 against 0.036.
...
- The utility gap of Section 6.1 cannot be explained by an inability to carry a conditional relationship. MST carries this one essentially perfectly on Diabetes 130 and with slope 0.946 on NHANES, where it still trains the random-forest student 0.086 AUC lower (Table 6); the 0.15 AUC gap of Section 6.1 is Adult's, where the protocol was not run.
...
The claim Stage C produces where it does clear is auditable and quantified: with probability at least 95%, simultaneously over the four released cells, which cover 25.4% of the population, the private and synthetic conditional rates differ by at most 0.092. The Stage C cost is ε = 1.0 on top of ε = 2.0 for the release, for ε = 3.0 per row and, on this one-row-per-person dataset, per person.
...
**Table 25:** Four attacks on four datasets, with a positive control on each. The attacked draws are the earlier-configuration arms of Section 5.5; the shadow attack was also run on shipped-configuration draws (Section 6.10). The positive control detects copying and does not calibrate the attacks against subtler leakage. Proposition 1's permitted advantage is for pure ε; the attacked arms' $(ε_L, δ)$ label with δ = 1.5 × 10⁻⁷ moves it by less than 10⁻⁶.
...
| marginal cost per 1,000 records | $1.03–$9.36 without the pool; $4.3–7.0 per 1,000 kept under the shipped configuration | free after fit | free after fit | free after fit |
...
Relabelling a marginal method's output using a private conditional table is pure post-processing of two already-private artefacts, so it costs nothing beyond the table. Table 28 measures it on Adult, five base draws per method crossed with three table draws for AIM and two for MST, under the same evaluator. Each private table is released at ε = 0.333, split equally across the levels it carries, on top of the base method's ε = 2, so every hybrid row is at ε = 2.333 per row. Relabelling moves AIM from TSTR-LR 0.675 to 0.777 and MST from 0.690 to 0.773. At level 4 the private table matches a non-private oracle to within 0.009, so the residual is draw-to-draw noise rather than a cost paid for privacy.
...
**Table 28:** The hybrid on Adult. Giving AIM and MST the quantity neither releases removes most of the gap to CoRTeC without altering either synthesiser's configuration.
...
And whether a private relationship survives synthesis is a property of the mechanism family, not of the guarantee. MST carries a forced two-way relationship as faithfully as CoRTeC does and, on the one dataset where both measurements exist, still trains a tree student 0.086 AUC lower; on Adult what it loses is measurable as a 2.8× inflation of pairwise feature dependence.

Where CoRTeC belongs is narrow and useful. It is the mechanism to use when a downstream model or a calibrated conditional rate is the product, and not the mechanism to use when a marginal report at scale is.
